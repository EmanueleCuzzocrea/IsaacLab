...
class AnymalCEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        if (self.learning_iteration < 1000):
            ...
            #mask_change_vel_p = (self.counter_vel[:, 0] > 200) & mask_p
            #self.counter_vel[mask_change_vel_p, 0] = 0
            #self._forces_reference[mask_change_vel_p] = torch.zeros_like(self._forces_reference[mask_change_vel_p]).uniform_(20.0, 60.0)
    ...
    def _get_rewards(self) -> torch.Tensor:        
        ...
        diag_1 = (self._robot.data.root_lin_vel_b[:, 0] + self._robot.data.root_lin_vel_b[:, 1])/1.4142
        diag_2 = (self._robot.data.root_lin_vel_b[:, 0] - self._robot.data.root_lin_vel_b[:, 1])/1.4142
        lin_vel_error_x = torch.square(self._commands[:, 0] - diag_1)
        lin_vel_error_mapped_x = torch.exp(-lin_vel_error_x / 0.15)
        # linear velocity tracking_y
        lin_vel_error_y = torch.square(self._commands[:, 1] - diag_2)
        ...
        mask_force = self._forces[:,0] > 0.001
        reward[mask_force] += 0.01
        reward2[mask_force] += 0.01
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if (self.count_int % 10 == 0):
            ...
            with open(file_path, 'w') as file:
                ...
                file.write(f"Learning iteration: {self.learning_iteration}\n")
        self.count_int += 1
        ...
        if (self.learning_iteration < 1000):
            ...
            self.mae = torch.mean(torch.abs(self._forces_reference[env_ids] - self._forces_buffer[env_ids]))
