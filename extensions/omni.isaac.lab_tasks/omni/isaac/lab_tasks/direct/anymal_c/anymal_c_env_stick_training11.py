...
class AnymalCEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        #    visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.1, 0.1, 0.1), metallic=0.2),
        #)
        #self._cuboid_cfg.func("/World/envs/env_.*/Cuboid", self._cuboid_cfg, translation=(1.25, 0.0, 0.5))
    ...
    def _get_observations(self) -> dict:
        print(self._forces[:,0])
        self._commands[:,1] += 0.005
    ...
    def _get_rewards(self) -> torch.Tensor:
        # posiiton tracking
        root_pos_ = self._robot.data.root_pos_w[:, :2] - self._terrain.env_origins[:, :2]
        target_pos_ = self._commands[:, :2]
        vel_w = self._robot.data.root_vel_w[:, :2]

        direction_vector = target_pos_ - root_pos_
        direction_vector = direction_vector / torch.norm(direction_vector)

        dot_product = torch.sum(vel_w * direction_vector, dim=1, keepdim=True)
        ...
        rewards = {
            ...
            "track_pos": dot_product.squeeze(1) * self.cfg.track_pos * self.step_dt,
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        x_ = random.uniform(0.6, 1.0)
        y_ = random.uniform(-1.0, 1.0)
        self._commands[env_ids,0] = 0.7
