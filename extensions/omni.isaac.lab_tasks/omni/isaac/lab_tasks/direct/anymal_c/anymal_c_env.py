...
class AnymalCFlatEnvCfg(DirectRLEnvCfg):
    ...
    # robot
    robot: ArticulationCfg = ANYMAL_KINOVA_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class AnymalCEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: AnymalCFlatEnvCfg | AnymalCRoughEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self._sequenza_target_2 = torch.tensor([0, 0, 0, 0, 0, 1], device=self.device)
        self._sequenza_target_3 = torch.tensor([0, 0, 1, 1, 0, 2], device=self.device)
        self._sequenza_target_4 = torch.tensor([0, 0, 0, 0, 0, 3], device=self.device)

        # Anymal and Kinova ids
        self._anymal_ids = [0,1,2,3,5,6,7,8,10,11,12,13]
        self._kinova_ids = [4,9,14,15,16,17]
    ...
    def _get_observations(self) -> dict:
        ...
        current_state = np.array([root_pos_[0][0].item(), root_pos_[0][1].item(), root_pos_[0][2].item(), yaw, 0.0, 0.0,
                                  self._robot.data.joint_pos[0,4].item(),
                                  self._robot.data.joint_pos[0,9].item(),
                                  self._robot.data.joint_pos[0,14].item(),
                                  self._robot.data.joint_pos[0,15].item(),
                                  self._robot.data.joint_pos[0,16].item(),
                                  self._robot.data.joint_pos[0,17].item(),
                                ])
        ...
        velocities = torch.tensor([control[6], control[7], control[8], control[9], control[10], control[11]], device=self.device)
        self._robot.set_joint_velocity_target(velocities, joint_ids=self._kinova_ids)
        ...
        obs = torch.cat(
            [
                ...
                for tensor in (
                    ...
                    self._commands,
                    self._robot.data.joint_pos[:,self._anymal_ids] - self._robot.data.default_joint_pos[:,self._anymal_ids],
                    self._robot.data.joint_vel[:,self._anymal_ids],
                    height_data,
                    self._actions,
                )
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        desiredStateTraj.push_back(np.array([3.0, 0.0, 1.2, 0.0, 0.7, 0.0, 0.7]))
        targetTrajectories = TargetTrajectories(desiredTimeTraj, desiredStateTraj, desiredInputTraj)
        mpc.reset(targetTrajectories)
        
        self.target_pos = torch.tensor([[3.0, 0.0, 1.2]], device=self.device)
        self.target_or = torch.tensor([[0.7, 0.0, 0.7, 0.0]], device=self.device)
